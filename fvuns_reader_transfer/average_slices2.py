...
def process_slices(input_pattern, output_file, dx=0.15625, dz=None, radius_limit=None, max_hole_dist=3.0, npy_output=None, start=None, end=None, step=None):
    """
    ...
    if start is not None or end is not None or step is not None:
        ...
        for f in files:
            ...
            file_nums.append(int(m.group()) if m else None)
        
        # Sort files by their extracted numbers
        sorted_indices = sorted(
            [i for i in range(len(files)) if file_nums[i] is not None],
            key=lambda i: file_nums[i]
        )
        
        base_start = start if start is not None else (file_nums[sorted_indices[0]] if sorted_indices else 0)
        ...
        for idx in sorted_indices:
            ...
            if end is not None and num > end:
                continue
            if step is not None:
                if (num - base_start) % step != 0:
                    continue
            filtered_files.append(files[idx])
        files = filtered_files
    else:
        import re
        files = sorted(files, key=lambda f: int(re.search(r'\d+', os.path.basename(f)).group()) if re.search(r'\d+', os.path.basename(f)) else f)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start", type=int, default=None, help="Start file index for filtering")
    parser.add_argument("--end", type=int, default=None, help="End file index for filtering")
    parser.add_argument("--step", type=int, default=None, help="Step/stride for filtering file indices")
